...
bool CubemapShader::Init(const std::vector<CubemapTexture>& textures)
{
	...
	if (!AddShader(GL_FRAGMENT_SHADER, "cubemap.fs.glsl"))
	{
		return false;
	}
...
{
	glUniform1i(glGetUniformLocation(program, "CubeMap"), 2);
...
bool CubemapShader::LoadTextures(const std::vector<CubemapTexture>& textures)
{
	...
	for (int i = 0; i < textures.size(); ++i)
	{
		...
		TextureData texData = Common::loadPng(textures[i].fileName.c_str());
